...
for (let name of folderNames) {
  fs.mkdir(
    path.join(__dirname, `../frontend/src/Assets/Villagers/${name}`),
    (err) => {
      if (err) throw err;
    }
  );
...
for (let i = 0; i < personalityTypes.length; i++) {
  for (let j = 0; j < personalityTypes[i].length; j++) {
    ...
    fs.rename(
      path.join(
        __dirname,
        `../frontend/src/Assets/Villagers/${currVillager}.png`
      ),
      path.join(
        __dirname,
        `../frontend/src/Assets/Villagers/${currFolder}/${currVillager}.png`
      ),
      (err) => {
        if (err) throw err;
