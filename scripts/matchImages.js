...
// engineering time would also take longer than doing it manually 
...
for (let i = 0; i < personalityTypes.length; i++) {
  ...
  fs.readdir(testFolder, (err, files) => {
    files.forEach(file => {
      filesArr.push(file.slice(0, -4));
    });

    let currSubset = [];

    for (let j = 0; j < personalityTypes[i].length; j++) {
      let currVillager = filesArr.find(villager => villager === personalityTypes[i][j]);

      if (currVillager === undefined) {
        currSubset.push(personalityTypes[i][j])
      }
    }
    missing[folderNames[i]] = currSubset
  });
}
